...
Source of truth: `docs/ARCHITECTURE.yaml`. Tracking: [Project #24](https://github.com/users/googa27/projects/24), [PizzaRecognition issue](https://github.com/googa27/arxiv-implementation-lab/issues/24). Profile: `legacy`; enforcement: `Advisory`.
...
- AI: Minimal AGENTS with upstream/revival guidance; no MCP/plugin.
- Human/notebook: Current script/demo API documented; typed model adapter only if independently revived.
- Planned Python protocols: No new dunders in the educational fork until dependency/security modernization and typed-package revival.
- Core posture: No core coupling; optionally emit generic model-output manifest after revival.
- Data posture: Legacy image data/model artifacts require license/provenance review before reuse.
